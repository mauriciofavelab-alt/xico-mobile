...
export default function EditInterestsScreen() {
  ...
  useEffect(() => {
    AsyncStorage.getItem("xico_interests").then(val => {
      if (val) { try { setSelected(JSON.parse(val)); } catch {} }
    });
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Clear cached narration-dependent content so next open re-fetches with new style
      await AsyncStorage.removeItem("xico_narration_style");
      await AsyncStorage.removeItem(`xico_companion_${TODAY}`);
    ...
    <View style={[ob.container, { paddingTop: topPad }]}>
      ...
      >
        <View style={ob.header}>
          <Text style={ob.eyebrow}>XICO · CRITERIO</Text>
          <Text style={ob.title}>Actualiza{"\n"}<Text style={ob.titleItalic}>tus intereses.</Text></Text>
          <View style={ob.divider} />
          <Text style={ob.subtitle}>Elige de dos a cuatro áreas. Tu catálogo y tu voz editorial se recalculan al guardar.</Text>
        </View>
      ...
      <View style={[ob.footer, { paddingBottom: Math.max(bottomPad + 16, 32) }]}>
        ...
        <Pressable
          ...
          style={({ pressed }) => [ob.cta, (!isReady || saving) && ob.ctaDisabled, pressed && isReady && { opacity: 0.8 }]}
        >
          <Text style={[ob.ctaText, (!isReady || saving) && ob.ctaTextDisabled]}>
            {saving ? "Guardando…" : isReady ? "Guardar cambios →" : `Elige ${Math.max(0, 2 - selected.length)} más`}
          </Text>
        </Pressable>
      </View>
    </View>
...
const ob = StyleSheet.create({
  ...
  scroll: { paddingTop: 8 },
  header: { paddingHorizontal: 28, paddingTop: 24, paddingBottom: 32 },
  eyebrow: {
